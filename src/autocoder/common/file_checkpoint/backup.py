...
class FileBackupManager:
    ...
    def clear_all_backups(self) -> None:
        ...
        """
        with self.lock:
            logger.info(f"开始清除所有备份数据于: {self.backup_dir}")
            
            # 删除所有单独的备份文件 (UUID命名的文件)
            # 同时保留 metadata 文件以便最后删除
            files_to_delete = []
            for item_name in os.listdir(self.backup_dir):
                item_path = os.path.join(self.backup_dir, item_name)
                if os.path.isfile(item_path) and item_name != os.path.basename(self.metadata_file):
                    # 检查是否为UUID格式的文件名，避免误删其他文件
                    try:
                        uuid.UUID(item_name, version=4)
                        files_to_delete.append(item_path)
                    except ValueError:
                        logger.debug(f"跳过非UUID格式的文件: {item_path}")
            
            for file_path in files_to_delete:
                try:
                    os.remove(file_path)
                    logger.debug(f"已删除备份文件: {file_path}")
                except Exception as e:
                    logger.warning(f"删除备份文件 {file_path} 失败: {str(e)}")
            
            # 删除元数据文件
            if os.path.exists(self.metadata_file):
                try:
                    os.remove(self.metadata_file)
                    logger.info(f"已删除备份元数据文件: {self.metadata_file}")
                except Exception as e:
                    logger.error(f"删除备份元数据文件 {self.metadata_file} 失败: {str(e)}")
            
            # 重置内存中的元数据
            self.metadata = {}
            
            # 重新创建空的元数据文件，以确保后续操作一致性
            self._save_metadata() 
            
            logger.info("所有备份数据已成功清除。")
