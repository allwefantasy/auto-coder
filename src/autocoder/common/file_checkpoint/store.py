...
class FileChangeStore:
    ...
    def clear_all_changes(self) -> None:
        """
        清除所有存储的变更记录，包括JSON文件和数据库。
        """
        with self.lock:
            try:
                logger.info(f"开始清除所有变更记录于: {self.store_dir}")
                ...
                for filename in os.listdir(self.store_dir):
                    if filename.endswith(".json"):
                        ...
                        try:
                            os.remove(file_path)
                            logger.debug(f"已删除变更记录文件: {file_path}")
                        except Exception as e:
                ...
                if os.path.exists(self.db_file):
                    try:
                        ...
                        logger.info(f"已删除变更数据库: {self.db_file}")
                    except Exception as e:
                        logger.error(f"删除数据库文件 {self.db_file} 失败: {str(e)}")
                
                # 重新初始化数据库
                self._init_db()
                logger.info("变更记录已成功清除并重新初始化数据库。")

            except Exception as e:
                logger.error(f"清除所有变更记录失败: {str(e)}")
                # 即使发生错误，也尝试重新初始化数据库以保证后续操作的稳定性
                try:
                    self._init_db()
                    logger.info("数据库已重新初始化。")
                except Exception as init_e:
                    logger.error(f"重新初始化数据库失败: {str(init_e)}")
                raise
