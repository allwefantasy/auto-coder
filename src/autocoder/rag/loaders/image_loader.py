...
def format_ocr_table(ocr_text: str) -> str:
    """
    根据OCR文本内容，自动识别疑似表格区域，将其格式化成markdown表格。
    输出符合replace_in_file工具格式的diff片段，可用于替换原始文本中的表格区域。

    Args:
        ocr_text: OCR识别得到的纯文本

    Returns:
        diff字符串，包含一个或多个search/replace块
    """
    import re
    ...
    while idx < len(non_empty_lines):
        ...
        if looks_like_table_row(line):
            ...
            if current_table:
                # 结束一段表格，处理
                original = "\n".join(non_empty_lines[start_idx:idx])

                # 转为markdown表格
                md = convert_to_markdown(current_table)
...
def convert_to_markdown(table_lines):
    import re
    rows = []
