...
try:
    ...
    response = query_sync("Write a complex algorithm", options)
    print(response)
...
```python
from autocoder.sdk.models import Message, MessageBatch, StreamEvent, CodeModificationResult

# 创建消息
message = Message(
    role="user",
    content="Write a Python class for file operations",
    metadata={"priority": "high"}
)

# 消息批次处理
batch = MessageBatch()
batch.add_user_message("First question", {"step": 1})
batch.add_assistant_message("First answer", {"model": "v3_chat"})
batch.add_user_message("Follow-up question", {"step": 2})
...
# 流式事件处理
start_event = StreamEvent.start_event(session_id="session-123")
content_event = StreamEvent.content_event("Generated code content")
end_event = StreamEvent.end_event(session_id="session-123")

# 代码修改结果
result = CodeModificationResult(
    success=True,
    message="Successfully created API endpoints",
    created_files=["api.py", "models.py"],
