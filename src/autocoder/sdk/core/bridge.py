    ...
    def call_run_auto_command(
        ...
        try:
            ...
            try:
                ...
                # 如果无法导入或出现其他错误，使用模拟模式
                yield from self._simulate_auto_command_response(query)
        ...
        try:
            ...
            if event_class_name == "LLMThinkingEvent":
                ...
                if hasattr(event, 'tool'):
                    tool_name = type(event.tool).__name__ if hasattr(event.tool, '__class__') else "Unknown"
                ...
                return StreamEvent(
                    event_type="error",
                    data={"error": getattr(event, 'message', 'Unknown error'), "error_type": "AgenticError"}
                )
