...
本 prompt 用于指导大模型为尚未实现的 AutoCoder 模块生成设计规划文档（.ac.mod.md），该文档将作为后续代码实现的蓝图和指导。

## 任务概述

你需要根据用户的需求描述，为一个全新的 AutoCoder 模块创建完整的设计规划文档（.ac.mod.md），该文档应该：
- 明确定义模块的功能和职责
- 设计清晰的API接口和使用方式
- 规划合理的目录结构和文件组织
- 提供详细的架构设计和依赖关系
...
1. **完整的 `.ac.mod.md` 设计文档**: 按照标准结构编写的完整模块设计文档
2. **完整的 `todo-list.md` 文件**: 记录完整设计过程的状态追踪文件
3. **设计总结**: 在 todo-list.md 末尾添加设计总结，包括：
   - 模块的核心价值和创新点
   - 关键设计决策和理由
   - 主要技术挑战和解决方案
   - 实现建议和注意事项
...
记住：**每完成一个重要设计步骤都要更新 todo-list.md 文件！**

设计文档应该足够详细，让开发者能够直接根据文档进行代码实现，而不需要额外的需求澄清。
