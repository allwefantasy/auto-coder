...
export function activate(context: vscode.ExtensionContext) {
	...
	let chatDisposable = vscode.commands.registerCommand('auto-coder.chat', () => {
		if (chatPanel) {
			...
			const scriptPathOnDisk = vscode.Uri.file(path.join(context.extensionPath, 'dist', 'web.js'));
			const scriptUri = chatPanel.webview.asWebviewUri(scriptPathOnDisk);
			const colorTheme = vscode.window.activeColorTheme;

			chatPanel.webview.html = getWebviewContent(scriptUri, colorTheme, 'chat');
					...
					switch (message.type) {
						case 'getAutoCoderServerPort':
							if (autoCoderServerPort) {
								if (chatPanel) {
									chatPanel.webview.postMessage({ type: 'autoCoderServerPort', port: autoCoderServerPort });
								}
							}
							return;
					}
